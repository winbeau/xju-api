...
web/dist/      — go:embed receiver for the built frontend (placeholder index.html
                 in git; real assets copied in by deploy/build-newapi.sh).
                 Frontend SOURCE lives at the repo top level: ../../web/
                 (React 19, Rsbuild, Base UI, Tailwind; single theme — the
                 classic theme and the bun workspace shell were removed in the
                 2026-07 top-level refactor, see docs/REFACTOR-PLAN.md §5.0)
```
...
### Frontend (`../../web/src/i18n/` — repo top level)
- Library: `i18next` + `react-i18next` + `i18next-browser-languagedetector`
- Languages: en (base), zh (fallback), fr, ru, ja, vi
- Translation files: `web/src/i18n/locales/{lang}.json` (repo top level) — flat JSON, keys are English source strings
- Usage: `useTranslation()` hook, call `t('English key')` in components
- CLI tools: `bun run i18n:sync` (from the top-level `web/`)
...
- Use `bun` as the preferred package manager and script runner for the frontend (top-level `web/`):
  - `bun install` for dependency installation
  - `bun run dev` for development server
  - `bun run build` for production build
  - `bun run i18n:*` for i18n tooling
- Frontend UI text must support i18n with `i18next`/`react-i18next`. Use flat JSON locale files in the top-level `web/src/i18n/locales/{lang}.json`, with English source strings as keys.
- In React components, use `useTranslation()` and call `t('English key')` for user-facing text.
- Follow the top-level `web/AGENTS.md` for detailed frontend conventions, including TypeScript, component structure, styling, accessibility, testing, and build checks.
