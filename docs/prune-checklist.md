...
- ⚠️ **代价（已写进 `deploy/run-newapi.sh`）**：空库首启不能再用浏览器建管理员，**只能走 `POST /api/setup`**。后端接口未改动，仅前端不再提供图形向导。

## 包 K — classic 主题 + bun workspace 壳（2026-07 顶层重组，REFACTOR-PLAN §5.0）

- [x] 删 `web/classic/` 整主题（React 18 + Vite + Semi Design，从未使用）与 workspace 壳（`web/package.json`、`web/bun.lock`）；前端源码上移为顶层 `web/`，package.json 转独立应用（catalog 版本内联、bun.lock 重生），`--filter './default'` 怪癖消失
- [x] 后端单主题化：`main.go` 删 classic 两组 embed 变量（embed 路径改 `web/dist`）；`router/web-router.go` `ThemeAssets`→`FrontendAssets`（单 FS）；`common/embed-file-system.go` 删 `themeAwareFileSystem`/`NewThemeAwareFS`；`common/constants.go` 主题初值 `classic`→`default`、`SetTheme` 只接受 `default`（线上 option 残值免疫）
- [x] 连带删除：上游 `makefile` / `Dockerfile` / `Dockerfile.dev` / `docker-compose{,.dev}.yml` / `electron/`（依赖旧前端布局，全部失效；git 历史可找回）、`.agents/skills/classic-to-default-sync`
