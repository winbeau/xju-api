...
- [ ] **Step 5: 建 K12 渠道 + 登记组**（**用户批准**）

> 部署期实况：claude-tri 上 `scripts/.env` **不存在**。先从 SQLite 取 root(winbeau,id=1)的 access_token
> 写一个 gitignored 的 `scripts/.env`（`**/.env` 已被挡）。若 root 无 access_token,用 [[model-pool-verification]]
> 的法子临时 SQL 设一个 char(32) access_token,用完保留即可（admin API 用）。

claude-tri：

```bash
# 取 root access_token + uid,写最小 scripts/.env（.env 已 gitignore,不入库）
UID_ROOT=1
TOK="$(python3 -c "import sqlite3;c=sqlite3.connect('/opt/new-api/data/one-api.db');print((c.execute('SELECT access_token FROM users WHERE id=1').fetchone() or [''])[0] or '')")"
if [ -z "$TOK" ]; then
  TOK="$(openssl rand -hex 16)"
  python3 -c "import sqlite3;c=sqlite3.connect('/opt/new-api/data/one-api.db');c.execute('PRAGMA busy_timeout=5000');c.execute('UPDATE users SET access_token=? WHERE id=1',('$TOK',));c.commit()"
  docker restart new-api >/dev/null   # 让 new-api 重载 DB（内存缓存)
  sleep 3
fi
cat > /home/winbeau/opt/xju-api/scripts/.env <<EOF
NEWAPI_BASE=http://127.0.0.1:3000
ACCESS_TOKEN=$TOK
NEWAPI_USER_ID=$UID_ROOT
EOF
chmod 600 /home/winbeau/opt/xju-api/scripts/.env
...
> 部署期实况：claude-tri **连不到 WSL:2222**。走 claude-vps 中转（本机同时可达 WSL 与 claude-tri）。
> 两跳都是 ssh 加密通道；临时文件用完即删，不落任何仓库目录。

本机（claude-vps）：

```bash
# WSL -> claude-vps
scp -i ~/.ssh/id_ed25519 -P 2222 "winbeau@100.78.122.53:/mnt/c/Users/genev/Desktop/alive500个K12.zip" /tmp/alive500.zip
# claude-vps -> claude-tri
scp -P 48687 /tmp/alive500.zip winbeau@70.39.193.15:/tmp/alive500.zip
rm -f /tmp/alive500.zip   # 本机中转副本用完即删
```
