#!/usr/bin/env bash
# scripts/toggle_card.sh — 临时关卡/开卡: PUT /api/token/?status_only=true(PLAN.md §4.2-③)
# 用法: ./toggle_card.sh <token_id> <on|off>
# 注意: 对「已过期」令牌 on 会被服务端拒绝 —— 复活请用 renew_card.sh(先续时再启用)
set -euo pipefail
cd "$(dirname "$0")"
...
if jq -e '.success' <<<"$RESP" >/dev/null; then
	...
	echo "操作失败: $RESP" >&2
	[[ "$ACTION" == on ]] && echo "提示: 若因已过期被拒,用 ./renew_card.sh $ID <天数> 复活" >&2
	exit 1
fi
